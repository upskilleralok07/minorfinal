...
if not os.path.exists(INTERNSHIP_CSV_PATH):
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    if st.session_state.internship_recommender is None:
        ...
        if manual_skills_input:
            manual_skills = [s.strip() for s in manual_skills_input.split(",") if s.strip()]
            if manual_skills:
        ...
        if parsed:
            ...
            st.write("**No resume analyzed yet. Add your skills manually below or upload a resume first.**")
            st.write(f"**Skills entered:** {len(user_skills)}")
        ...
        with col2:
            ...
            max_distance = st.slider("Maximum Distance (km)", 0, 600, 200)
            work_from_home = st.checkbox("Only Work From Home opportunities", value=False)
        ...
        if st.session_state.internship_recommendations is not None and not st.session_state.internship_recommendations.empty:
            ...
            for _, row in recommendations.iterrows():
                ...
                <div class='card'>
                    ...
                    <p><b>Location:</b> {str(row['location']).title()}</p>
